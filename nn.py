...
class Graph(object):
    ...
    def __init__(self, variables):
        ...
        """
        """
        self.maps is a list of maps containing inputs, outputs, & gradient, each corresponding to one node
        self.indices is a map between nodes and (node, index) tuples, where index is the index in self.maps 
            where we can find that node's map
        self.nodes is an ordered list of the nodes to return with get_nodes
        """
        #the new
        self.outputs = {}
        self.gradients = {}
    ...
    def get_inputs(self, node):
        ...
        """
        return [self.outputs[parent] for parent in node.get_parents()]
    ...
    def add(self, node):
        ...
        """
        forward_val = node.forward(self.get_inputs(node))
        self.outputs[node] = forward_val
        self.gradients[node] = np.zeros_like(forward_val)
    ...
    def backprop(self):
        ...
        self.gradients[loss_node] = np.array([1.0])
        for node in self.get_nodes()[::-1]:
            if not isinstance(node, Variable) and not isinstance(node, Input):
                self.backpropHelper(node)

    def backpropHelper(self, node):
        parents = node.get_parents()
        pre_grad = self.get_gradient(node)
        grad = node.backward(self.get_inputs(node), pre_grad)

        self.gradients[parents[0]] = self.gradients[parents[0]] + grad[0]
        if not isinstance(node, ReLU):
            self.gradients[parents[1]] = self.gradients[parents[1]] + grad[1]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        a = inputs[0]
        b = inputs[1]
        return [(a - b) / a.shape[0] * gradient, (b - a) / b.shape[0] * gradient]
